...
async function ProfileSection() {
    ...
    const data = await GetInfo(queries.infosUser)
    if (!VerifyError(data)) return

    console.log(data)
    const cohort = data?.data?.user?.[0]?.events?.[0]?.cohorts?.[0]?.labelName || ''
    const login = data.data.user[0].login
    const info = data.data.user[0].attrs
    const useravatar = data.data.user[0].avatarUrl
    const memberSince = data.data.user[0].createdAt
        ? new Date(data.data.user[0].createdAt).toLocaleDateString()
        : ''

    const address = [info.addressStreet, info.addressCity, info.addressPostalCode, info.addressRegion, info.addressCountry]
        .filter(Boolean)
        .join(', ')
    const arabicName = [info.arabicFirstName, info.arabicLastName].filter(Boolean).join(' ')
    ...
    userInfoSec.innerHTML = `
            ...
            <div class="profile-avatar">
                <img src="${useravatar || ''}" alt="profile pic" class="profile-pic"/>
            </div>
            <div class="username">
            ...
            <div class="info-data">
                ...
                <p><span>Phone:</span> ${info.tel || "--"}</p>
                <p><span>Gender:</span> ${info.gender || "--"}</p>
                <p><span>City:</span> ${info.addressCity || "--"}</p>
                <p><span>Address:</span> ${address || "--"}</p>
                <p><span>Place of Birth:</span> ${info.placeOfBirth || "--"}</p>
                <p><span>Birth Country:</span> ${info.birthCountry || "--"}</p>
                <p><span>Arabic Name:</span> ${arabicName || "--"}</p>
                <p><span>Member Since:</span> ${memberSince || "--"}</p>
            </div>
        `
    container.appendChild(userInfoSec)

    userInfoSec.querySelector('.close-sec').addEventListener('click', () => {
        document.querySelector('.user-info-section').style.transform = 'translate(100%)'
    })
}
